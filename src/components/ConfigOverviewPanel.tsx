...
function ConfigOverviewPanel({ overview }: ConfigOverviewPanelProps) {
  ...
  return (
    <div className="space-y-4">
      {/* File path */}
      <div className="flex items-center gap-2 text-xs text-dark-200 bg-dark-800/50 px-3 py-2 rounded-lg">
      ...
      <Section title="Inbounds" icon={<ArrowRightLeft size={15} />}>
        {overview.inbounds.map((inbound, idx) => (
          <div
            key={idx}
            className="flex items-center justify-between p-2.5 bg-dark-800/50 rounded-lg"
          >
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-white">{inbound.tag}</span>
            </div>
            <span className="text-xs text-dark-200">{inbound.details}</span>
          </div>
        ))}
      </Section>
      ...
      {groups.length > 0 && (
        <Section title="Outbound Groups" icon={<Shield size={15} />}>
          {groups.map((group, idx) => (
            <div key={idx} className="p-2.5 bg-dark-800/50 rounded-lg space-y-1.5">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-white font-medium">{group.tag}</span>
                </div>
                <span className="text-xs text-dark-200">{group.details}</span>
              </div>
              <div className="flex flex-wrap gap-1.5 pl-1">
                {group.group_members.map((member, mIdx) => (
                  <span
                    key={mIdx}
                    className="text-[11px] bg-dark-700 text-dark-100 px-2 py-0.5 rounded"
                  >
                    {member}
                  </span>
                ))}
        ...
        {proxyNodes.map((node, idx) => (
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-white">{node.tag}</span>
            </div>
            <div className="text-right">
              ...
              <p className="text-[11px] text-dark-200/70">{node.details}</p>
            </div>
          </div>
        ))}
      </Section>
          ...
          {overview.dns_servers.map((dns, idx) => (
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-white">{dns.tag}</span>
              </div>
              <span className="text-xs text-dark-200">{dns.server}</span>
            </div>
          ))}
        </div>
      </Section>
          ...
          <div className="grid grid-cols-2 gap-1.5">
            {overview.rule_sets.map((rs, idx) => (
              ...
              >
                ...
                <span className="text-xs text-white truncate">{rs.tag}</span>
              </div>
            ))}
          </div>
        </Section>
...
function StatCard({
  ...
  return (
    <div className="bg-dark-800/50 border border-dark-700/50 rounded-lg p-3 text-center">
      <div className={`flex justify-center mb-1 ${color}`}>{icon}</div>
      <p className="text-lg font-bold text-white">{value}</p>
      <p className="text-[10px] text-dark-200">{label}</p>
    </div>
  );
}
